...
window.letterKinds = window.letterKinds || {};
...
const graphicButtons = document.getElementById('graphic-buttons');
const addLetterButton = document.getElementById('add-letter-button');
const addGraphicButton = document.getElementById('add-graphic-button');
const newLetterRow = document.getElementById('new-letter-row');
const newItemModeLabel = document.getElementById('new-item-mode-label');
...
function getItemType(name) {
    if (!name) return draftItemType;
    return window.letterKinds?.[name] || inferStoredItemType(name);
}

function isGraphicItem(name) {
    return getItemType(name) === 'graphic';
}
...
function updateLetterSelectionUi() {
    ...
    const activeItemType = isExistingLetter ? getItemType(currentLetterName) : draftItemType;
    const itemLabel = activeItemType === 'graphic' ? 'graphic' : 'letter';

    if (isExistingLetter) {
        selectedLetterStatus.textContent = `Editing ${itemLabel} "${currentLetterName}".`;
        saveButton.textContent = 'Save changes';
    } else if (newLetterRow.classList.contains('hidden')) {
        selectedLetterStatus.textContent = 'Pick a letter or graphic to edit, or create a new one.';
        saveButton.textContent = 'Save changes';
    } else {
        const typedCharacter = newLetterInput.value.trim();
        selectedLetterStatus.textContent = typedCharacter
            ? `Creating new ${itemLabel} "${typedCharacter}".`
            : activeItemType === 'graphic'
                ? 'Type the name of the graphic you want to design.'
                : 'Type the character you want to design.';
        saveButton.textContent = activeItemType === 'graphic' ? 'Save new graphic' : 'Save new letter';
    }
...
function openNewItemMode(type = 'letter') {
    ...
    setPaletteNotice(type === 'graphic'
        ? 'Create a reusable graphic by giving it a name like "heart" or "star", then draw it on the grid and save.'
        : 'Create a reusable single-character letter, then draw it on the grid and save.');
...
function updateNewLetterInputAttributes() {
    if (draftItemType === 'graphic') {
        newItemModeLabel.textContent = 'New graphic';
        newLetterInput.placeholder = 'e.g. heart, smiley, star';
        newLetterInput.removeAttribute('maxlength');
    } else {
        newItemModeLabel.textContent = 'New letter';
...
function rebuildLettersFromSnapshot(querySnapshot) {
    ...
    querySnapshot.forEach((letterDoc) => {
        ...
        window.letterKinds[letterDoc.id] = inferStoredItemType(letterDoc.id, data);
        if (data.gridData) {
...
function getSelectedColorLabel(color) {
    ...
    return entry.label === entry.color ? entry.color : `${entry.label} ${entry.color}`;
}
...
function setEditorEnabled(enabled) {
    [newLetterInput, cancelNewLetterButton, addLetterButton, addGraphicButton, saveButton, deleteButton, downloadButton, paletteColorInput, offsetXInput, offsetYInput, offsetApplyButton, offsetResetButton].forEach((element) => {
...
function loadLetter(char) {
    ...
    previousLetterName = '';
    draftItemType = getItemType(char);
    closeNewLetterMode();
    initEditor(true);
    refreshLetterButtons();
    updateLetterSelectionUi();
...
function refreshLetterButtons() {
    ...
    graphicButtons.innerHTML = '';

    Object.keys(window.letters || {}).sort().forEach((char) => {
        const button = document.createElement('button');
        const isGraphic = isGraphicItem(char);
        button.className = `builder-letter-btn${isGraphic ? ' builder-graphic-btn' : ''}${currentLetterName === char ? ' active' : ''}`;
        button.innerText = char;
        button.addEventListener('click', () => loadLetter(char));
        (isGraphic ? graphicButtons : letterButtons).appendChild(button);
    });
}
...
window.addToAlphabet = async function addToAlphabet() {
    ...
    if (!name) {
        window.alert(draftItemType === 'graphic'
            ? 'Please type the name of the graphic you want to save first.'
            : 'Please type the character you want to save first.');
        return;
    }

    const isGraphicMode = currentLetterName ? isGraphicItem(currentLetterName) : draftItemType === 'graphic';
    ...
    try {
        ...
        await setDoc(doc(db, 'letters', name), {
            ...
            colorData,
            offsetData,
            itemType: isGraphicMode ? 'graphic' : 'letter'
        });
        ...
        window.letterKinds[name] = isGraphicMode ? 'graphic' : 'letter';
...
window.deleteLetter = async function deleteLetter() {
    ...
    if (!name) {
        window.alert('Please load an existing letter or graphic to delete.');
        return;
    }

    const itemLabel = isGraphicItem(name) ? 'graphic' : 'letter';
    const confirmed = window.confirm(`Delete the ${itemLabel} "${name}"? It will be removed from your website and this cannot be undone.`);
    if (!confirmed) {
        return;
    }
    ...
    try {
        ...
        delete window.letterKinds[name];
        currentLetterName = '';
        previousLetterName = '';
        closeNewLetterMode();
        clearEditorGrid();
        ...
        refreshLetterButtons();
        updateLetterSelectionUi();
        window.alert(`${itemLabel[0].toUpperCase() + itemLabel.slice(1)} "${name}" has been deleted.`);
    } catch (error) {
...
addLetterButton.addEventListener('click', () => {
    openNewItemMode('letter');
});

addGraphicButton.addEventListener('click', () => {
    openNewItemMode('graphic');
});
